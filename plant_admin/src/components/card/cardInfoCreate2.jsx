...
const CardInfo = () => {
    ...
    const createPlant = () => {
        ...
        tab2?.cultivar != ""
            ? // tab2?.name_ru&&
              ...
              // tab2?.link_synonym
              axiosInstance
                  .post("/api/create-plant", { ...tab1, ...tab2 })
                  .then((res) => {
    ...
    const getPlant = (id) => {
        let data;
        axiosInstance
            .get("/api/get-admin-plant-by-id/" + id)
            .then((res) => {
                console.log("plant data obj", res.data);
                setPlant(res.data);
                data = res.data;
            })
            .catch((err) => {
    ...
    const createMorfology = () => {
        axiosInstance
            .post("/api/create-morphology", tab5)
            .then((res) => {
                console.log(res.data);
                message.success(res.data.msg);
            })
        ...
        <div className="esasyDiv" style={{ width: "100%" }}>
            ...
            <button className="createBtn" style={{top:"10px"}}>Create+</button>
                ...
                <h1 onClick={()=>setIndex(2)} className={index==2&&"activeTabHeader"}>Названия</h1> */}
                    <h1
                        onClick={() => setIndex(3)}
                        className={index == 3 && "activeTabHeader"}
                    >
                        Ареалы
                    </h1>
                    <h1
                        onClick={() => setIndex(4)}
                        className={index == 4 && "activeTabHeader"}
                    >
                        Карта
                    </h1>
                    <h1
                        onClick={() => setIndex(5)}
                        className={index == 5 && "activeTabHeader"}
                    >
                        Морфология
                    </h1>
                    <h1
                        onClick={() => setIndex(6)}
                        className={index == 6 && "activeTabHeader"}
                    >
                        Экология
                    </h1>
                    <h1
                        onClick={() => setIndex(7)}
                        className={index == 7 && "activeTabHeader"}
                    >
                        Применение
                    </h1>
                    <h1
                        onClick={() => setIndex(8)}
                        className={index == 8 && "activeTabHeader"}
                    >
                        Дополнительно
                    </h1>
                    <h1
                        onClick={() => setIndex(9)}
                        className={index == 9 && "activeTabHeader"}
                    >
                        Гербарий
                    </h1>
                    <h1
                        onClick={() => setIndex(10)}
                        className={index == 10 && "activeTabHeader"}
                    >
                        Рисунки
                    </h1>
                    <h1
                        onClick={() => setIndex(11)}
                        className={index == 11 && "activeTabHeader"}
                    >
                        Текст
                    </h1>
                </div>
                ...
                <div className="tabBody">
                    ...
                    {index == 4 && (
                        <div className="tabChild">
                            <Tab4 getPlant={getPlant} data={plant?.maps} />
                            <div style={{ marginLeft: "84%", width: "100%" }}>
                                <button
                                    onClick={() => setIndex(3)}
                                    style={{
