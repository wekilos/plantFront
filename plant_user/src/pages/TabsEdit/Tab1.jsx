...
const Tab1 = (props)=>{
    ...
    const getSubClass = ()=>{
        axiosInstance.get("/api/get-admin-subclass").then((res)=>{
            console.log(res.data);
            Array.isArray(res.data)?setSubClasses(res.data):setSubClasses([])
        }).catch((err)=>{
            console.log(err);
        })
    }
    ...
    const getSuperSubClass = ()=>{
        axiosInstance.get("/api/get-admin-supersubclass").then((res)=>{
            console.log(res.data);
            Array.isArray(res.data)?setSuperSubClasses(res.data):setSuperSubClasses([])
        }).catch((err)=>{
            console.log(err);
        })
    }
    ...
    const getSubOrders = ()=>{
        axiosInstance.get("/api/get-admin-suborder").then((res)=>{    
            Array.isArray(res.data)?setSubOrders(res.data):setSubOrders([])
        }).catch((err)=>{
            console.log(err);
        })
    }
    ...
    const getFamily = ()=>{
        axiosInstance.get("/api/get-admin-families").then((res)=>{    
            Array.isArray(res.data)?setFamilies(res.data):setFamilies([])
            Array.isArray(res.data)&&res.data.map((item)=>{props?.data?.family_id==item.id&&setFamilyAwtor(item)}); 
        }).catch((err)=>{
            console.log(err);
        })
    }
